...
public class XnModalPanel : MonoBehaviour {
    ...
    public void ButtonPress(Button bPressed) {
        ...
        if (num == -1) {
            Debug.LogError("Button pressed that was not in List<> buttons. You probably need to set up the buttons again.");
            return;
        }
        showing = false;
    ...
    public void Show(string msg, System.Action<int> callback = null, string b0text = "Ok", string b1text = "", string b2text = "") {
        text = msg;
        _callback = callback;
        string[] inStrings = { b0text, b1text, b2text };
        for (int i = 0; i < buttons.Count; i++) {
            if (inStrings[i] != "") { // button 0 is always visible by default with the b0text = "Ok" default parameter
    ...
    static public void SHOW(string msg, System.Action<int> callback = null, string b0text = "Ok", string b1text = "", string b2text = "") {
        if (S == null) {
