...
public class CutsceneDialogueManager : MonoBehaviour
{
    ...
    {
        StartCoroutine("HandleRemoveTextInstance");
    }
    ...
    IEnumerator HandleRemoveTextInstance()
    {
        TextElement scrollViewText = (TextElement)doc.rootVisualElement.Q<VisualElement>("unity-content-container").ElementAt(0).ElementAt(0);
        while (scrollViewText.text.Length > 0)
        {
            audioSource.Play();
            scrollViewText.text = scrollViewText.text.Substring(0, scrollViewText.text.Length - 1);
